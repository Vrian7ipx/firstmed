			...
			<div class="col-md-4">
				<div class="row">
					...
					{{-- {{ Former::legend('datos de Producto') }} --}}
						<p >
							<label>C贸digo*</label>
							<input type="text" name="product_key" class="form-control" placeholder="C贸digo del Producto" aria-describedby="sizing-addon2" title="Ingrese C贸digo del Producto" pattern="^[a-zA-Z0-9-].{1,}" required   value="{{$product->product_key}}">
						</p>
					</div>
				</div>

		      	{{-- {{ Former::text('product_key')->label('C贸digo')->title('Solo se acepta Letras, N煤meros y gui贸n(-).') }} --}}
		      	<p>
		      ...
		      {{-- 	{{ Former::select('Unidad')->options('', '')->fromQuery(Unidad::all(), 'nombre', 'id')
				...
				<div class="row">
					<div class="col-md-5">
						<label>Precio *</label>
					    <input class="form-control" type="text" name="cost" placeholder="Precio del Producto" aria-describedby="sizing-addon2" required title="Solo se acepta n鷐eros. Ejem: 500.00" pattern="[0-9]+(\.[0-9][0-9]?)?" required value="{{$product->cost}}">				      	
					</div>
				</div>
